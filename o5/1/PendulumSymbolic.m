...
p{2} = p{1} + L*[sin(theta2), -cos(theta2)]';
...
Lag_q = simplify(jacobian(Lag,q)).';        % partial L partial q
Lag_qdq = simplify(jacobian(Lag_q.',dq));   % partial L^2 partial q dq
Lag_dq = simplify(jacobian(Lag,dq)).';      % partial L partial dq
Lag_dqdq = simplify(jacobian(Lag_dq.',dq)); % parital L^2 partial dq dq
